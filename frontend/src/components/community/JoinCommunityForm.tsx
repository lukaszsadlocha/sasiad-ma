...
const JoinCommunityForm: React.FC = () => {
  ...
  const onSubmit = async (data: JoinCommunityRequest) => {
    try {
      ...
      toast.success(t('community.join.successMessage'));
      navigate('/communities');
    } catch (error: any) {
      toast.error(error.message || t('community.join.errorMessage'));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-md mx-auto space-y-6">
      ...
      <div className="text-center">
        ...
        <h1 className="text-2xl font-bold text-gray-900">{t('community.join.title')}</h1>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div>
          <label htmlFor="invitationCode" className="block text-sm font-medium text-gray-700">
            {t('community.join.codeLabel')}
          </label>
          <div className="mt-1 relative">
            <Key className="absolute left-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-gray-400" />
            <input
              {...register('invitationCode', {
                required: t('community.join.codeRequired'),
                minLength: {
                  value: 3,
                  message: t('community.join.codeMinLength')
                }
              })}
              type="text"
...
{t('community.join.joinButton')}
      ...
      <div className="text-center text-sm text-gray-500">
        <p>{t('community.join.noCode')}</p>
        <button
          onClick={() => navigate('/communities/create')}
          className="text-blue-600 hover:text-blue-500 font-medium"
        >
          {t('community.join.createOwn')}
        </button>
      </div>
    </div>
  );
